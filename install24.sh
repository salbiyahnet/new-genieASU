...
echo -e "${GREEN}============================================================================${NC}"
echo -e "${GREEN}Sebelum melanjutkan, apakah Anda ingin melanjutkan? (y/n)${NC}"
read confirmation
...
# Info UI
echo -e "${GREEN}========== GenieACS UI port 3000: http://$local_ip:3000 ============${NC}"
...
if [ -d "new-genieASU" ]; then
    ...
    rm -rf new-genieASU
    echo -e "${GREEN}Restore database selesai.${NC}"
else
    echo -e "${RED}Folder 'new-genieASU' tidak ditemukan. Restore database dibatalkan.${NC}"
fi
